...
<head>
    ...
    <title>{{ __('Sign-in is temporarily unavailable') }}</title>
    <style>
...
<body>
    <div class="card">
        <h1>{{ __('Sign-in is temporarily unavailable') }}</h1>
        <p>{{ __('We could not reach the identity provider. Please try again in a few minutes.') }}</p>
        <a href="{{ $loginUrl }}">{{ __('Try again') }}</a>
    </div>
</body>
</html>
